...
file_dir <- "/share/clean/status_30min.csv"
system.time({  status <- read.csv(file_dir, sep = ",", header = TRUE)  })
...
file_dir <- "/share/clean/status_30min_flo.csv"
system.time({  status <- read.csv(file_dir, sep = ",", header = TRUE)  })

file_dir1 <- "/share/clean/retNum.csv"
file_dir2 <- "/share/clean/borNum.csv"
system.time({  retNum <- read.csv(file_dir1, sep = ",", header = TRUE)  })
system.time({  borNum <- read.csv(file_dir2, sep = ",", header = TRUE)  })
...
file_dir1 <- "/share/clean/trip_fix.csv"
file_dir2 <- "/share/clean/station.csv"
wea_dir <- "/share/clean/weather.csv"

trip <- read.csv(file_dir1, sep = ",", header = TRUE)
station <- read.csv(file_dir2, sep = ",", header = TRUE)
weather <- read.csv(wea_dir, sep = ",", header = TRUE)

a <- as.character(trip$start_date)
trip$id <- paste0(substr(a, 1, 4), substr(a, 6, 7), substr(a, 9, 10), substr(a, 12, 13))

trip["k"] <- 1:nrow(trip)

trip2 <- merge(trip, station, by.x = "start_station_id", by.y = "station_id", all.x = TRUE)
trip2 <- merge(trip2, weather, by = c("id","city"), all.x = TRUE)

trip2 <- trip2[order(trip2$k),]

write.table(trip2, "/share/clean/trip_two.csv", sep = ",", row.names = FALSE)
