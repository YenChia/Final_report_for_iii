...
file_dir <- "/share/clean/status_1_two.csv"
...
#                          fill = TRUE)
#})
system.time({  status <- read.csv(file_dir, sep = ",", header = FALSE)  })
...
write.table(status, file = "/share/clean/status_2_two.csv", row.names = FALSE, col.names = FALSE)
...
for (i in 3:14) {
  ...
  write.table(status2, file = savefile, sep = ",", row.names = FALSE)
}
end_time <- Sys.time()
end_time - start_time
